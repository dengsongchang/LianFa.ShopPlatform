...
! function ($) {
	...
	bTabs.prototype.init = function () {
		...
		$('ul.wrapper-nav-tabs a', $($tabs)).each(function (i, row) {
		...
		//为关闭图标绑定事件，并适用于动态新增的
		$('ul.wrapper-nav-tabs', $tabs).on('click', 'button', function (e) {
			var id = $(this).parent().attr('href').replace('#', '');
			self.closeTab(id);
		});
		//处理窗口拖拽排序功能
		if (p.sortable && $.fn.sortable) {
			$('ul.wrapper-nav-tabs', $tabs).sortable({
	...
	bTabs.prototype.innerResize = function () {
		...
		var tabBarHeight = $('ul.wrapper-nav-tabs', $tabs).outerHeight(true);
	...
	bTabs.prototype.addTab = function (id, title, url, loginCheck) {
		...
		if (openTabs && $.isArray(openTabs) && openTabs.length > 0) {
			...
			//若功能已存在，则直接切换
			if (exist) {
				$('ul.wrapper-nav-tabs a[href="#' + tabId + '"]', $tabs).tab('show');
				var currentIndex = 0;
                //获取当前选中项距离最左边的距离
                var AcurrentLeft = 0;
                $('.wrapper-nav-tabs').find('li').each(function(index,item){
                    if($(item).hasClass('active')){
                        AcurrentLeft = $(item).position().left;
                        console.log("选中项距离左边的偏移量",$(item).position().left)
                    }
                })
                //可见内容区域宽度
                ...
                //计算偏移量和可视区域的倍数关系
                //向上取整
                var number = Math.ceil(Math.abs(AcurrentLeft) / showWidth)
				//当前页数
		...
		$('ul.wrapper-nav-tabs', $tabs).append('<li><a href="#' + tabId + '" data-toggle="tab">' + title + c.closeBtnTemplate + '</a></li>');
		var content = $('<div class="tab-pane" id="' + tabId + '"></div>');
		$('#navTabsContent', $tabs).append(content);
		//切换到新增加的tab上
		$('ul.wrapper-nav-tabs li:last a', $tabs).tab('show');
        ...
        $('.wrapper-nav-tabs').find('li').each(function(index,item){
            if($(item).hasClass('active')){
                AcurrentLeft = $(item).position().left;
                console.log("新增选中项距离左边的偏移量",$(item).position().left)
            }
        })
        //可见内容区域宽度
        var showWidth = $('.containerHeadBox').width() - $('.J_tabLeft').width() - $('.J_tabRight').width();
	...
	bTabs.prototype.closeTab = function (id) {
		...
		//移除内容区
		$(thisTab).remove();
		var a = $('ul.wrapper-nav-tabs a[href="#' + id + '"]', $tabs);
		var li = $(a).closest('li');
		//获得当前tab的前一个tab
		var prevLi = $(li).prev();
		//移除Tab
        ...
        $('.wrapper-nav-tabs').find('li').each(function(index,item){
            if($(item).hasClass('active')){
                AcurrentLeft = $(item).position().left + $(item).width();
                console.log("新增选中项距离左边的偏移量",$(item).position().left)
            }
        })
        //可见内容区域宽度
