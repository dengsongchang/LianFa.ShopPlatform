var navSettingList = {
    init: function () {
        ...
        $('body').on('change','.order-disp',function(){
            ...
            navSettingList.adminChangeNbDisplayOrder(AdId,DisplayOrder)
        });
        //删除按钮点击
  ...
  adminChangeNbDisplayOrder:function (AdId,DisplayOrder) {
    ...
    adminBatchDelNavigationBar:function (list) {
        //请求方法
        var methodName = "/adverts/AdminDelPopUpAdverts";
        var data = {
            AdIdList:list
        };
    ...
    initBootstrapTable: function () {
        $('#brand_box').bootstrapTable({
            method: 'post',
            url: SignRequest.urlPrefix + '/adverts/AdminPopUpAdvertsList',
            dataType: "json",
            ...
            columns: [{
                ...
                },
                {
                    ...
                    valign: 'middle',
                    formatter: function (value, row, index) {
                        var html = `<input type="number" style="text-align: center" class="order-disp" data-id="${row.AdId}" value="${value}">`
                        return html;
                    }
                },
                {
                    ...
                    valign: 'middle',
                    formatter: function (value, row, index) {
                        var html = '<a class="editor" href="/homePage/popEdit?id=' + value + '">编辑</a>';
                        html +='<a class="delectBtn" style="margin-left: 5px" data-id="'+row.AdId+'" href="javascript:void(0)">删除</a>'
                        return html;
                    }
                },
    ...
    queryParams: function (params) {
        //配置参数
        //方法名
        var methodName = "/adverts/AdminPopUpAdvertsList";
    ...
    responseHandler: function (res) {
        if (res.Data != null) {
            console.log(res);
            return {
                "rows": res.Data.AdminPopUpAdvertsList,
