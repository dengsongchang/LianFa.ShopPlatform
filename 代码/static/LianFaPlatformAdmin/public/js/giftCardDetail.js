...
var ProductRelease = {
    ...
    init: function () {
        ...
        uploadIconPic('#product_upload_pick', '#product_icon', '/product/AdminUploadProductImg');
    ...
    getProductInfo: function (PId) {
        var methodName = "/Coupon/AdminGetCouponDetail";
        var data = {
            CouponId: PId
        };
        SignRequest.setAsync(methodName, data, function (data) {
            console.log(data)
            if (data.Code == "100") {
                $('#cardName').val(data.Data.Name)
                $('#SerialNumber').val(data.Data.CouponSn)
                $('#userName').val(data.Data.UserName)
                $('#buyTime').val(data.Data.AddTimes)
                $('#checkTime').val(data.Data.UseTimes)
                $('#state').val(data.Data.StateDec)
                $('#orderNumber').val(data.Data.OSn)
                $('#orderState').val(data.Data.OrderStateDec)
