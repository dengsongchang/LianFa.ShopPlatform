...
var PopEdit = {
    init: function () {
        ...
        $('#choicePerformanceModal').on('shown.bs.modal', function () {
            PopEdit.initPerformanceBootstrapTable()
        });
        //选择品牌专场模态框出现获取数据
        $('#choiceBrandModal').on('shown.bs.modal', function () {
            PopEdit.initBootstrapTable()
        });
        ...
        $('body').on('click', '#performanceBtn', function () {
            PopEdit.projectPerformanceQuery();
        })
        //上传小图标
        // uploadIconPic('#small_upload_pick', '#small_icon', '/adverts/AdminUploadAdvertsImg');
        // PopEdit.adminGetAdvert()
        //保存按钮点击
        $('body').on('click', '#nextStep', function () {
            var Icon = $('#small_icon').attr('data-src');
            //广告名称
            if (!Validate.emptyValidateAndFocus("#Title", "请输入广告名称", "")) {
                return false;
            }
            //是否选择了类型
            if(!$('#type').val()){
            ...
            //开始时间
            if ($('#start_time').val() == null || $('#start_time').val() == undefined) {
                Common.showInfoMsg('请选择开始时间')
                return false;
            }
            //结束时间
            if ($('#end_time').val() == null || $('#end_time').val() == undefined) {
                Common.showInfoMsg('请选择结束时间')
                return false;
            }
    ...
    //获取弹窗广告信息
    adminPopUpAdvertsInfo: function () {
        //请求方法
        var methodName = "/adverts/AdminPopUpAdvertsInfo";
        var data = {
            AdId:Common.getUrlParam('id'),
        };
        //请求接口
        SignRequest.set(methodName, data, function (data) {
            console.log(data)
            if (data.Code == "100") {
                if(data.Data.AdminPopUpAdvertsInfo.Image != ""){
                    $('#small_icon').attr('data-src',data.Data.AdminPopUpAdvertsInfo.Image)
                    $('#small_icon').attr('src',data.Data.AdminPopUpAdvertsInfo.ImageFull)
                }
                $('#type').val(data.Data.AdminPopUpAdvertsInfo.Type)
                if(data.Data.AdminPopUpAdvertsInfo.Type == 1){
                    //商品
                    $('#productBox').show();
                    $('#performanceBox').hide();
                    $('#BrandBox').hide();
                    $('.goods-name_final').show();
                    $('.goods-name_final').text(data.Data.AdminPopUpAdvertsInfo.Name)
                    $('.goods-name_final').attr('data-id',data.Data.AdminPopUpAdvertsInfo.Id)
                }else if(data.Data.AdminPopUpAdvertsInfo.Type == 2){
                    $('#productBox').hide();
                    $('#performanceBox').show();
                    $('#BrandBox').hide();
                    $('.performance-name_final').show();
                    $('.performance-name_final').text(data.Data.AdminPopUpAdvertsInfo.Name)
                    $('.performance-name_final').attr('data-id',data.Data.AdminPopUpAdvertsInfo.Id)
                }else if(data.Data.AdminPopUpAdvertsInfo.Type == 3){
                    $('#productBox').hide();
                    $('#performanceBox').hide();
                    $('#BrandBox').show();
                    $('.brand-name_final').show();
                    $('.brand-name_final').text(data.Data.AdminPopUpAdvertsInfo.Name)
                    $('.brand-name_final').attr('data-id',data.Data.AdminPopUpAdvertsInfo.Id)
                }

                $('#start_time').val(data.Data.AdminPopUpAdvertsInfo.StartTime)
                $('#end_time').val(data.Data.AdminPopUpAdvertsInfo.EndTime)
                $('#DisplayOrder').val(data.Data.AdminPopUpAdvertsInfo.DisplayOrder);
                $('#Title').val(data.Data.AdminPopUpAdvertsInfo.Title);
                // 状态
                var stateList = $("#State label");
                for (var i = 0; i < stateList.length; i++) {
                    if (stateList.eq(i).find("input").val() == data.Data.AdminPopUpAdvertsInfo.State) {
    ...
    preHandle:function(){
        if(PopEdit.viewer){
            PopEdit.viewer.destroy();
        }
        PopEdit.viewer = new Viewer(document.getElementById('dowebok'),{
            url: 'data-original',
            show:function(){
                PopEdit.viewer.update();
            },
        });
    ...
    //后台编辑弹窗广告
    adminEditPopUpAdverts: function () {
        var data = {
            "AdId": Common.getUrlParam('id'),
        ...
        //请求方法
        var methodName = "/adverts/AdminEditPopUpAdverts";
        data.Id = 0
        ...
        SignRequest.set(methodName, data, function (data) {
            console.log(data)
            if (data.Code == "100") {
                Common.showInfoMsg('编辑成功',function(){
                    location.href = '/homePage/popList'
                })
    ...
    initPerformanceBootstrapTable: function () {
        $('#choice_performance_tb').bootstrapTable({
            ...
            clickToSelect: true, //点击行即可选中单选/复选框
            sidePagination: "server", //表格分页的位置
            queryParams: PopEdit.queryPerformanceParams, //参数
            queryParamsType: "limit", //参数格式,发送标准的RESTFul类型的参数请求
            toolbar: "#toolbar", //设置工具栏的Id或者class
            responseHandler: PopEdit.responsePerformanceHandler,
            columns: [
    ...
    initBootstrapTable: function () {
        $('#tb_limit_content').bootstrapTable({
            ...
            clickToSelect: true, //点击行即可选中单选/复选框
            sidePagination: "server", //表格分页的位置
            queryParams: PopEdit.queryParams, //参数
            queryParamsType: "limit", //参数格式,发送标准的RESTFul类型的参数请求
            toolbar: "#toolbar", //设置工具栏的Id或者class
            responseHandler: PopEdit.responseHandler,
            columns: [
