var Configure = {
    ...
    init:function(){
        //初始化表格
        Configure.initBootstrapTable();
        Configure.getProductCategory();//初始化商品分类
        Configure.getProductBrand(); //初始化商品品牌
        //关闭弹窗
        ...
        $("#productSearch").on("click",function(){
            ...
            Configure.projectSelectQuery(data);
        });
        //点击选择商品
        $(".content").on("click","#selectProduct",function () {
            $(".mask").show();
            Configure.selectBootstrapTable();
        })
        //排序
        $('body').on('change','.order-disp',function(){
            var id = $(this).attr('data-id');
            var num = $(this).val();
            Configure.editConfigure(id,num);
        })
        //表格分页每页显示数据
        $("#pagesizeDropdown").on("change",function () {
            Configure.projectQuery();
        });
        //选择商品表格分页每页显示数据
        $("#pagesize_dropdown").on("change",function () {
            Configure.productDestoryQuery();
        });
        // 配置全选
        ...
        $(".content").on("click",".status_delete",function () {
            var id = $(this).attr('data-id');
            Configure.delProductConfig(id);
        })
        //多个删除
        $("#delete").on("click",function(){
            if(Configure.getSelectedData().length > 0){
                Common.confirmDialog("确认对选中的数据进行删除吗？",function(){
                    Configure.batchDelProductConfig(Configure.getSelectedData());
                });
            }
            else {
    ...
    //    点击添加按钮
        $("#addProduct").on("click",function(){
            if(Configure.getSelectedProductData().length > 0){
                Configure.AddChooseProduct(Configure.getSelectedProductData());
            }
            else {
    ...
    //    点击一键添加按钮
        $("#bathAdd").on("click",function(){
            ...
            if(list.length != 0){
                Configure.allAddChooseProduct(name,cateId,brandId);
            } else {
    ...
    getProductCategory:function(){
        ...
        SignRequest.set(methodName, data, function (data) {
            if (data.Code == "100") {
                var render = template.compile(Configure.cateTpl);
    ...
    getProductBrand:function(){
        ...
        SignRequest.set(methodName, data, function (data) {
            if (data.Code == "100") {
                var render = template.compile(Configure.brandTpl);
                var html = render(data.Data);
                $("#BrandId").append(html);
            } else {
                Common.showErrorMsg(data.Message);
    ...
    initBootstrapTable: function () {
        $('#configureTable').bootstrapTable({
            ...
            queryParams: Configure.queryParams, //参数
            queryParamsType: "limit", //参数格式,发送标准的RESTFul类型的参数请求
            toolbar: "#toolbar", //设置工具栏的Id或者class
            responseHandler: Configure.responseHandler,
            columns: [
                {
                    field: 'PId',
                    title: '',
    ...
    selectBootstrapTable: function () {
        $('#productTabel').bootstrapTable({
            ...
            clickToSelect: true, //点击行即可选中单选/复选框
            sidePagination: "server", //表格分页的位置
            queryParams: Configure.selectQueryParams, //参数
            queryParamsType: "limit", //参数格式,发送标准的RESTFul类型的参数请求
            toolbar: "#toolbar", //设置工具栏的Id或者class
            responseHandler: Configure.selectResponseHandler,
            columns: [
                {
                    field: 'PId',
                    title: '',
