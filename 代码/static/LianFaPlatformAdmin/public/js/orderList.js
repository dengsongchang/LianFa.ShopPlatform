...
`,
    ...
    init: function () {
        ...
        $('body').on('click', '.tabs_orderList_ul li', function () {
            ...
            if(uid){
                location.href = '/order/orderList?type=' + state + '&UId='+ uid + ''
            }else{
        ...
        //推送按钮点击
        $('body').on('click','#pushOrder',function(){
        ...
        $('body').on('click','#orderConfirm',function(){
            if (!Validate.emptyValidateAndFocus("#orderNumber", "请输入订单号", "")) {
                return false;
            }
            OrderList.adminRetailOrderBack();
        })

        // //回调按钮点击
        // $('body').on('click','.huidiaoBtn',function(){
        //     $('.mask').show();
        //     $('.checkBox').show();
        //     localStorage.setItem('oid', $(this).attr('data-id'));
        //
    ...
    //推送订单接口
    adminRetailOrderBack: function () {
        var methodName = "/order/AdminRetailOrderBack";
        var data = {
            "OSn": $('#orderNumber').val(),
        };
        SignRequest.set(methodName, data, function (data) {
            if (data.Code == "100") {
               Common.showSuccessMsg('推送订单成功',function(){
        ...
        $('#orderBox').bootstrapTable({
            ...
            columns: [

                {
                    ...
                    formatter: function (value, row, index) {
                        ...
                        var e = '<span style="display:block;position: relative">' +
                            // '<input type="checkbox" data-id="'+row.OId+'" class="order_checkbox" style="position: absolute;left: 7px;top: 3px;">' +
                ...
                },
                {
                    ...
                    formatter: function (value, row, index) {
                        var e =
                            '<span class="sorting_1"style="display:block;height:20px;">' + row.PayfriendName + '</span>';
                    ...
                    formatter: function (value, row, index) {
                        if (row.OrderState == "20") {
                            var html =
                                ...
                                '<span style="padding: 0 6px;color:#44b8fd;cursor: pointer" data-state="' + 160 + '" class="closeOrder actionBtn" data-id="' + value + '">关闭订单</span>';
    ...
    queryParams: function (params) {
        ...
        if (OrderList.flag) {
            var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
                ...
                Mobile:$('#Mobile').val(),
                oSn: $('#order_num').val(),
                consigNee: $('#receive_person').val(),
                orderState: Number(Common.getUrlParam('type')) ? Common.getUrlParam('type') : $('#state_box').val(),
                userName: $('#account_name').val(),
                SellStoreSn: $('#goods_name').val(),
                startTime: $('#start_time').val(),
                endTime: $('#end_time').val(),
                RegionId: OrderList.regionid,
                orderSoirce: $('#orderTarget').val(),
                ProvinceId: OrderList.provinceId,
    ...
    exportAdminOrdesList: function (list) {
        var methodName = "/order/ExportAdminOrdesList";
        var data = {
            oSn: $('#order_num').val(),
            consigNee: $('#receive_person').val(),
            orderState: Number(Common.getUrlParam('type')) ? Common.getUrlParam('type') : $('#state_box').val(),
