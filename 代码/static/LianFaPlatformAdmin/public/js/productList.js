...
var ProductList = {
    ...
    init: function () {
        ...
        //销量更改
        $('body').on('change', '.order-dispNumber', function () {
            var id = $(this).attr('data-id');
            var index = $(this).val();
            ProductList.adminEditProductVirtualSales(id, index)
        })
        ...
        $("#productTable").on("click", ".status_edit", function () {
            ...
            location.href = "/product?PId=" + PId + "&type=" + type;
        ...
        $("#onSaleBtn").on("click", function () {
            Common.confirmDialog("确认要一键上架?", function () {
                if (ProductList.getSelectedData().length == 0) {
                    Common.showErrorMsg("请选择商品", function () {
                    });
                } else {
                    ProductList.onSaleProduct(ProductList.getSelectedData(), true);
                }
            });
        });
        //一键下架
        $("#outSaleBtn").on("click", function () {
            Common.confirmDialog("确认要一键下架？", function () {
                if (ProductList.getSelectedData().length == 0) {
                    Common.showErrorMsg("请选择商品", function () {
                    });
                } else {
                    ProductList.outSaleProduct(ProductList.getSelectedData(), true);
                }
            });
        });
        ...
        //库存
        $("#productTable").on("click", ".status_num", function () {
            var PId = $(this).attr("data-id");
        ...
        ProductList.getProductBrand();
        ProductList.getProductLabel();
    ...
    //后台编辑商品排序
    adminEditProductVirtualSales: function (PId, VirtualSales) {
        var methodName = "/product/AdminEditProductVirtualSales";
        var data = {
            "PId": PId,
    ...
    getProductBrand: function () {
        ...
        SignRequest.set(methodName, data, function (data) {
            if (data.Code == "100") {
                var render = template.compile(ProductList.brandTpl);
                var html = render(data.Data);
                $("#BrandId").append(html);
            } else {
                Common.showErrorMsg(data.Message);
    ...
    getProductLabel: function () {
        ...
        SignRequest.set(methodName, data, function (data) {
            if (data.Code == "100") {
                var render = template.compile(ProductList.labelTpl);
                var html = render(data.Data);
                $("#Label").append(html);
            } else {
                Common.showErrorMsg(data.Message);
