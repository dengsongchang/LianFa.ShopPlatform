    ...
    <div class="allBox">
        ...
        <div class="addressBox" @click="toAddress">
            ...
            <div class="addressLeft" v-if="defaultFullShipAddressInfo && !shipAddressInfo">
                ...
                <div class="leftBottom">{{defaultFullShipAddressInfo.ProvinceName}}{{defaultFullShipAddressInfo.CityName}}{{defaultFullShipAddressInfo.CountyName}}{{defaultFullShipAddressInfo.Address}}</div>
            </div>
            <!--选择了其他地址-->
        ...
        <div class="productBox">
            ...
            <!--商家-->
            <div class="storeBox">
                <div class="giftcard_tltle" v-for="item,index in CartProductList" :key="index" v-if="item.IsSelected">
                    <div class="giftcard_neme">
                        {{item.OrderProductInfo.Name}}
                    </div>
                    <div class="giftcard_img">
                        <img :src="item.OrderProductInfo.ShowImg" alt="">
                    </div>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            confirmOrderHandle: function() {
                ...
                //直接购买
                var methodName = '/OrderHandel/ConfirmBuyCardOrder'
                var requestData = {
            ...
            submitOrderHandle: function() {
                ...
                var local = encodeURIComponent('' + that.GLOBAL.urlPrefixNoAdmin + '/middlePage?type=gift');
                //支付宝支付
                if (that.payType == 1) {
                    if (that.isWeChat) {
            ...
            submitOrder: function() {
                ...
                that.postData(methodName, requestData).then(function(data) {
                    ...
                    if (data.data.Code == "100") {
                        ...
                        if (that.payType == 2 && !that.isWeixin) {
                            console.log(data)
                                //h5端发起微信支付
...
<style scoped src="../../static/css/giftPayOrder.css">
