...
using LianFa.ShopPlatform.Code.Json;
...
namespace LianFa.ShopPlatform.Code.Config
{
    public class ConfigHelper
    {
        ...
        /// <param name="configPath">配置节路径，如：configuration/HostBaseUrl</param>
        /// <returns></returns>
        public static List<string> ReadConfigList(string configFileName, string configPath)
        {
            ...
            try
            {
                ...
                if (File.Exists(configUrl))
                {
                    xmlDoc.Load(configUrl);
                    {
                        var node = xmlDoc.SelectSingleNode(configPath);
                        if (node != null)
                        {
                            result?.AddRange(from XmlNode cNode in node.ChildNodes select cNode.InnerText);
                        }
                    }
                }
                else
